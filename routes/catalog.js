...
let query = "SELECT product_id, productname, prodimage, description, suppliername, cost_per_ounce, catagory FROM product, supplier where product.supplier_id=supplier.supplier_id"; 
// execute query
...
router.get('/cart', function(req, res, next) {
    ...
    let query = "SELECT product_id, productname, prodimage, description, supplier_id, cost_per_ounce, catagory FROM product WHERE product_id IN (" + req.session.cart + ") order by find_in_set(product_id, '" + req.session.cart + "');";
    // execute query
    db.query(query, (err, result) => {
...
router.get('/checkout', function(req, res, next) {
        ...
        if (typeof req.session.customer_id !== 'undefined' && req.session.customer_id ) {
            ...
            let insertquery = "INSERT INTO saleorder(customer_id, saledate, paymentstatus, authorizationnum) VALUES (?, now(), 'Paid', '12345678')"; 
            db.query(insertquery,[req.session.customer_id],(err, result) => {
                ...
                // Obtain the order_id value of the newly created SALEORDER Record
                    ...
                    req.session.cart.forEach((cartitem, index) => { 
                        ...
                        let insertquery = "INSERT INTO orderdetail(order_id, product_id, saleprice, container_size_in_ounces) VALUES (?, ?, (SELECT cost_per_ounce from product where product_id = " + cartitem + "), ?)";
